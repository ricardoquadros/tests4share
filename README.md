## Tests I have done or been given to do

### Suggestions are welcome

    * org.examples.stream.ReturnHighestValue
        - Return the Max value between 3 parameters
            - getIndexWithHigherValue(int val0, int val1, int val2)
            - getIndexWithHigherValue1(int val0, int val1, int val2)

    * org.examples.collections.MoveProtonsNeutrons
        - The method takes 4 parameters, protonStart and neutronStart represent the 
        number of protons in the initial atom and the protonTarget and the neutronTarget 
        represent the number of protons and neutrons in the desired atom. Return a list of strings

